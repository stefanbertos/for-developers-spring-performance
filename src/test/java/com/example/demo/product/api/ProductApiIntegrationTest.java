...
class ProductApiIntegrationTest {
	...
	void productApiEndToEndTest() throws Exception {
		// Create a product
		ProductRequest createRequest = new ProductRequest("Integration Test Product",
				"This is a product created during integration testing", new BigDecimal("123.45"), "Test Category",
				"https://example.com/integration-test.jpg", true);
		...
		// Update the product
		ProductRequest updateRequest = new ProductRequest("Updated Integration Test Product",
				"This product has been updated during integration testing", new BigDecimal("199.99"),
				"Updated Test Category", "https://example.com/updated-integration-test.jpg", false);

		mockMvc
			.perform(put("/api/v1/products/{id}", productId).contentType(MediaType.APPLICATION_JSON)
	...
	void createProduct_WithInvalidData_ShouldReturnBadRequest() throws Exception {
		...
		ProductRequest invalidRequest = new ProductRequest("", // Empty name (invalid)
	...
	void getProduct_WithInvalidId_ShouldReturnNotFound() throws Exception {
		mockMvc.perform(get("/api/v1/products/{id}", 999999)).andExpect(status().isNotFound());
	}

	@Test
	void createProduct_WithDuplicateName_ShouldReturnBadRequest() throws Exception {
		// Create a product
		ProductRequest createRequest = new ProductRequest("Duplicate Name Test Product",
				"This is a product created to test duplicate name validation", new BigDecimal("123.45"),
				"Test Category", "https://example.com/duplicate-test.jpg", true);
