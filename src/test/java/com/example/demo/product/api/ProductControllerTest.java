...
class ProductControllerTest {
    ...
    void getProductsByCategory_ShouldReturnPageOfProductsInCategory() {
        ...
        // Act
        ResponseEntity<PageResponse<ProductResponse>> response = productController.getProductsByCategory(category, 0, 10, "id", "asc");
    ...
    void searchProductsByName_ShouldReturnPageOfProductsWithNameContaining() {
        ...
        // Act
        ResponseEntity<PageResponse<ProductResponse>> response = productController.searchProductsByName(name, 0, 10, "id", "asc");
    ...
    void getProductById_WhenProductDoesNotExist_ShouldReturnNotFound() {
        // Arrange
        Long id = 999L;
        when(productService.getProductById(id)).thenThrow(new EntityNotFoundException("Product not found with ID: " + id));
    ...
    void createProduct_WhenNameExists_ShouldReturnBadRequest() {
        // Arrange
        when(productService.createProduct(productRequest)).thenThrow(new IllegalArgumentException("Product with name '" + productRequest.name() + "' already exists"));
    ...
    void updateProduct_WhenProductDoesNotExist_ShouldReturnNotFound() {
        // Arrange
        Long id = 999L;
        when(productService.updateProduct(id, productRequest)).thenThrow(new EntityNotFoundException("Product not found with ID: " + id));
    ...
    void updateProduct_WhenNameExistsForDifferentProduct_ShouldReturnBadRequest() {
        // Arrange
        Long id = 1L;
        when(productService.updateProduct(id, productRequest)).thenThrow(new IllegalArgumentException("Another product with name '" + productRequest.name() + "' already exists"));
