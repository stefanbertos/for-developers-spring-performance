...
class ProductController {
    ...
    @GetMapping("/search")
    @Operation(summary = "Search products by name", description = "Returns a paginated list of products with names containing the search term")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved products",
                    content = @Content(schema = @Schema(implementation = PageResponse.class)))
    })
    ...
    @PutMapping("/{id}")
    @Operation(summary = "Update an existing product", description = "Updates an existing product and returns the updated product")
    @ApiResponses(value = {
